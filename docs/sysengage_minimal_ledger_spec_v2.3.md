...
- `cell_id: string`` (REQUIRED)
  ...
  Canonical format: ZC-R{row}-C-{column} — e.g. ZC-R1-C-What, ZC-R3-C-How

- `row: string`` (REQUIRED)
  Description: Zachman row index.
  Notes: MUST be one of: "1", "2", "3", "4", "5", "6"

  Canonical values (1–6):

   | Value |	Perspective label (for reference only, not normative) |
   | -------- | ------- |
   | "1" |	Contextual / Planner / Scope|
   | "2" |	Conceptual / Owner / Business Concept|
   | "3" |	Logical / Designer / System Logic|
   | "4" |Physical / Builder / Technology Physics|
   | "5" 	|Detailed / Programmer / Component Assembly|
   | "6"|	Operational / User / Enterprise|

- `column: string`` (REQUIRED)
  Description: Zachman column identifier.
  Notes: MUST be one of: "What", "How", "Where", "Who", "When", "Why"

  Canonical values:

   |Value	|Interrogative meaning|
   | -------- | ------- |
   |"What"	|Data / Information|
   |"How"|	Function / Process|
   |"Where"	|Network / Location|
   |"Who"	|People / Organisation|
   |"When"	|Time / Schedule|
   |"Why"	|Motivation / Goal|
...
- `cell_id` MUST follow the pattern ZC-R{row}-C-{column} where {row} is the numeric row value and {column} is the exact interrogative string value.
- `row` MUST NOT be empty.
- `row` MUST be one of: "1", "
...
- No two `ZachmanCell` elements within the same ledger MAY share the same (`row`, `column`) coordinate pair. At most one `ZachmanCell` SHALL exist per grid position.
...
3. `sysengage_ledger_version` MUST equal the specification version of this ledger model (e.g., "2.3").
...

{
   ...
   "title":"SysEngage Ledger Instance (v2.3) — Canonical Serialization",
   "type":"object",
   ...
   "$defs":{
      ...
      "element_type":{
         ...
         "description":"Permitted element types (ledger spec v2.3)."
      },
      "id_S":{
         "type":"string",
         "pattern":"^S\\d{3}$"
